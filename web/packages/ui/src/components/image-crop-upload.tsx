...
import { Upload, X, ZoomIn, ZoomOut, RotateCw, Check, Crop } from "lucide-react"
        ...
        <div className="px-5 pb-5 space-y-4">
          ...
          <div className="flex gap-2">
            {[-90, -45, 0, 45, 90].map((deg) => (
              <button
                key={deg}
                onClick={() => setCrop((c) => ({ ...c, rotation: deg }))}
          ...
          <div className="flex gap-2 pt-1">
            <button
              onClick={() => setCrop({ x: 0, y: 0, zoom: 1, rotation: 0 })}
...
// ─── Main ImageCropUpload component ───────────────────────────────────────────
            ...
            {uploading ? (
              ...
              <>
                ...
                <p className="text-[10px] text-muted-foreground">
                  You'll be able to crop & zoom before uploading
                </p>
              </>
            )}
          </div>
        ...
        {displaySrc && (
          <div className="relative overflow-hidden rounded-lg border border-border">
            ...
            <div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-black/60 px-3 py-2">
              <span className="text-xs text-white/60 truncate max-w-[70%]">
                {value.split("/").pop()}
              </span>
...
// Re-export plain version for backward compat
