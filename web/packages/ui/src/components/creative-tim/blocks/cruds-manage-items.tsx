...
async function fetchItems(): Promise<Item[]> {
  try {
    const res = await fetchWithAuth("http://localhost:5103/api/items", {
      method: "GET",
    })
...
async function fetchTypes(): Promise<ItemType[]> {
  try {
    const res = await fetchWithAuth("http://localhost:5103/api/items/types", {
      method: "GET",
    })
    if (!res.ok) throw new Error(`${res.status}`)
...
export default function CrudsManageItems() {
  ...
  const load = async () => {
    const [fetchedItems, fetchedTypes] = await Promise.all([
      fetchItems(),
      fetchTypes(),
    ])
  ...
  const validate = (f: ItemForm) => {
    ...
    if (
      !f.validityDays ||
      isNaN(Number(f.validityDays)) ||
      Number(f.validityDays) < 1
    )
      return "Validity days must be ≥ 1."
    return null
  }
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await fetchWithAuth("http://localhost:5103/api/admin/items", {
        method: "POST",
  ...
  const handleUpdate = async (id: number) => {
    ...
    try {
      const res = await fetchWithAuth(
        `http://localhost:5103/api/admin/items/${id}`,
        {
          method: "PUT",
  ...
  const handleDeactivate = async (id: number) => {
    if (!confirm("Deactivate this item? It will no longer be purchasable."))
      return
    try {
      const res = await fetchWithAuth(
        `http://localhost:5103/api/admin/items/${id}`,
      ...
      flash("Failed to deactivate item.")
    }
  }
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Create, edit and deactivate ticket passes.
            </p>
          </div>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="shrink-0 rounded bg-primary/10 px-1.5 py-0.5 text-xs text-primary capitalize">
                        {item.typeName}
                      </span>
                    </div>
                    <p className="mt-0.5 text-sm text-muted-foreground">
                      {item.price} HUF · {item.validityDays} day
                      {item.validityDays !== 1 ? "s" : ""}
                    </p>
                    {item.description && (
