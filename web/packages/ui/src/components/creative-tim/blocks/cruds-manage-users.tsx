...
const ROLE_COLORS: Record<string, string> = {
  admin: "bg-purple-500/30 text-purple-400",
  staff: "bg-amber-500/30 text-amber-400",
  user: "bg-blue-500/30 text-blue-400",
}
      ...
      <Card className="mx-auto w-full max-w-2xl p-6 lg:p-8 dark:bg-black/20">
        <div className="mb-6 flex items-start justify-between">
          ...
          {foundingAdminId !== null && (
            <div className="flex items-center gap-1.5 rounded-md border border-purple-500/30 bg-amber-500/10 px-2.5 py-1.5 text-xs text-amber-600 dark:text-amber-400">
          ...
          <div className="container mx-auto flex-row items-start gap-6">
            {data.map(
              ({ id, name, username, email, role, isActive, createdAt }) => {
                ...
                const roleBadgeCls =
                  ROLE_COLORS[role] ?? "bg-zinc-500/20 text-zinc-400"
                return (
                  <Collapsible
                    key={id}
                  ...
                  >
                    <CollapsibleTrigger className="flex w-full items-center justify-between rounded p-2 transition-colors hover:bg-muted/50">
                      ...
                      <div className="flex items-center gap-2 text-xs text-muted-foreground">
                        ...
                        <span
                          className={`rounded-full px-2 py-0.5 font-semibold capitalize ${isActive ? "bg-green-500/20 text-green-600 dark:text-green-400" : "bg-red-500/20 text-red-500"}`}
                        >
                          {isActive ? "Active" : "Inactive"}
                        </span>
                        <span
                          className={`rounded-full px-2 py-0.5 font-semibold capitalize ${roleBadgeCls}`}
                        >
                          {role}
                        </span>
                      </div>
