  ...
  return (
    <Animated.View style={{ transform: [{ scale }] }}>
      ...
      >
        ...
        >
          <View className="absolute -top-2.5 w-5 h-5 rounded-full" style={{ backgroundColor: bgColor }} />
          <View className="w-2 h-2 rounded-full" style={{ backgroundColor: statusColor }} />
          <Text
            className="text-[11px] font-extrabold"
            style={{ color: isDark ? "#71717a" : "#a1a1aa", transform: [{ rotate: "-90deg" }] }}
          >
            #{item.orderId}
          </Text>
        ...
        <View className="flex-1 p-3.5 justify-between">
          ...
          <View className="flex-row gap-5 mt-2">
            <View className="gap-0.5">
              <Text className="text-[10px] font-semibold tracking-[1px]"
          ...
          <View
            ...
            style={{ backgroundColor: statusColor + "18", borderColor: statusColor + "40" }}
          >
        ...
        <View
          ...
          style={{ borderLeftColor: isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.06)" }}
        >
          <Text className={`text-[22px] ${isDark ? "text-[#71717a]" : "text-[#a1a1aa]"}`}>▦</Text>
        </View>
      </TouchableOpacity>
    </Animated.View>
  );
...
export default function OfflineTicketsScreen() {
  ...
  useEffect(() => {
    getOfflineTickets().then(setTickets).finally(() => setLoading(false));
  }, []);
  ...
  return (
    ...
    >
      ...
      <View pointerEvents="none" className="absolute -top-[30px] -right-[30px] w-[180px] h-[180px] rounded-full"
      ...
      <View className="px-5 pt-16 pb-5">
        ...
        <Text className={`text-[30px] font-extrabold tracking-[-0.5px] ${isDark ? "text-[#fafafa]" : "text-[#09090b]"}`}>
          {t("tickets.title")}
        </Text>
        ...
        <View className="flex-row items-center gap-3 mt-1.5 flex-wrap">
          ...
          <View className="flex-row items-center gap-1.5 px-2.5 py-1 rounded-full border"
            ...
            <Text className="text-[#f87171] text-[11px] font-bold tracking-[0.5px]">OFFLINE</Text>
          </View>
          ...
          {tickets.length > 0 && (
            <>
              <View className="flex-row items-center gap-1.5">
                ...
                <Text className={`text-xs ${isDark ? "text-[#a1a1aa]" : "text-[#52525b]"}`}>
        ...
        {tickets.length > 0 && tickets[0].savedAt && (
          <Text className={`text-[11px] mt-1.5 ${isDark ? "text-[#71717a]" : "text-[#a1a1aa]"}`}>
            Cached {new Date(tickets[0].savedAt).toLocaleDateString(undefined, {
      ...
      {loading ? (
        ...
        <View className="flex-1 items-center justify-center gap-3.5 px-10">
          <Text className={`text-xl font-bold text-center ${isDark ? "text-[#fafafa]" : "text-[#09090b]"}`}>
            {t("tickets.noTickets")}
          </Text>
          <Text className={`text-xs text-center leading-5 ${isDark ? "text-[#a1a1aa]" : "text-[#52525b]"}`}>
        ...
        >
          {active.length > 0 && (
            <>
              <Text className={`text-[11px] font-bold tracking-[2px] uppercase mb-1 ${isDark ? "text-[#71717a]" : "text-[#a1a1aa]"}`}>
                {t("tickets.activeLabel")}
              </Text>
              {active.map((item) => (
                <TicketCard key={item.id} item={item} isDark={isDark} t={t} onPress={() => navigateToDetail(item)} />
              ))}
            </>
          )}
          {expired.length > 0 && (
            <>
              <Text className={`text-[11px] font-bold tracking-[2px] uppercase mt-4 mb-1 ${isDark ? "text-[#71717a]" : "text-[#a1a1aa]"}`}>
