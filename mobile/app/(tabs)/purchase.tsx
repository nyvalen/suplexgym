    ...
    <Animated.View style={{
      ...
      <LinearGradient colors={["#7c3aed", "#4f46e5"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
        ...
        <TouchableOpacity onPress={() => router.push("/purchase-finalization")} activeOpacity={0.85}
          ...
          <Text style={{ color: "#fff", fontSize: 14, fontWeight: "700" }}>{t("purchase.checkout")}</Text>
        </TouchableOpacity>
      </LinearGradient>
    </Animated.View>
  );
...
export default function PurchaseTicketsScreen() {
  ...
  const addToCart = useCallback(async (item: TicketItem) => {
    ...
    try {
      ...
      if (res.ok) addItem({ itemId: item.id, name: item.name, price: item.price, validityDays: item.validityDays, typeName: item.typeName, type_id: item.type_id, quantity: 1 });
    } catch { } finally { setAdding((p) => ({ ...p, [item.id]: false })); }
  }, [adding, addItem, cart]);

  const removeFromCart = useCallback(async (item: TicketItem) => {
    ...
    try {
      ...
      if (cartRes.ok) {
        ...
        if (si) await authFetch(ENDPOINTS.cartItem(si.id), { method: "DELETE" });
      }
